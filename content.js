...
export const mcqPool = [
  ...
  { id:'q7', topic:'methods', q:'You need every sample to have a value for a peptide that was fragmented once. Which design guarantees that?', choices:['LFQ (separate runs)','DIA (wide windows)','TMT (isobaric, one pooled run)','SILAC (2–3 channels)'], answer:2, explain:'TMT pools samples into ONE run, so a single MS2 gives a reporter value for every channel — no cross-sample missingness within the plex (LFQ/DIA missingness is a cross-run problem).' },
  ...
  { id:'q25', topic:'missing', q:'A value missing because the instrument simply did not sample that peptide that time is…', choices:['MNAR (missing not at random)','MCAR / MAR (random)','A decoy hit','An imputed value'], answer:1, explain:'Random, abundance-independent gaps are MCAR/MAR — unlike MNAR, where the peptide is truly too low to detect.' },
